...
int _mixBytes(int s0, int s1, int s2, int s3) {
  int t3, t2, t1, t0;
  t0 = _esub[(s0 >>> 24) & 0xFF];
  t1 = _esub[(s1 >>> 16) & 0xFF];
  t2 = _esub[(s2 >>> 8) & 0xFF];
...
void wordTable() {
  ...
  for (int x = 0; x < 256; ++x) {
    int t = _esub[x];
    int y = (_mul2[t] << 24) ^ (t << 16) ^ (t << 8) ^ (_mul3[t]);
    if (x & 3 == 0) {
      if (x == 4) stdout.write('//');
      stdout.write('\n  ');
    }
  ...
  for (int x = 0; x < 256; ++x) {
    int t = _esub[x];
    int y = (_mul3[t] << 24) ^ (_mul2[t] << 16) ^ (t << 8) ^ (t);
    if (x & 3 == 0) {
      if (x == 4) stdout.write('//');
      stdout.write('\n  ');
    }
  ...
  for (int x = 0; x < 256; ++x) {
    int t = _esub[x];
    int y = (t << 24) ^ (_mul3[t] << 16) ^ (_mul2[t] << 8) ^ (t);
    if (x & 3 == 0) {
      if (x == 4) stdout.write('//');
      stdout.write('\n  ');
    }
  ...
  for (int x = 0; x < 256; ++x) {
    int t = _esub[x];
    int y = (t << 24) ^ (t << 16) ^ (_mul3[t] << 8) ^ (_mul2[t]);
    if (x & 3 == 0) {
      if (x == 4) stdout.write('//');
      stdout.write('\n  ');
    }
...
void byteTable() {
  stdout.write('const List<int> _mul2 = [');
  for (int x = 0; x < 256; ++x) {
    if (x & 7 == 0) {
      if (x == 8) stdout.write('//');
      stdout.write('\n  ');
    }
    int y = (x & 0x80) == 0 ? (x << 1) : ((x << 1) ^ 0x1b);
    y &= 0xff;
    var hex = y.toRadixString(16).padLeft(2, '0');
    stdout.write('0x$hex, ');
  }
  stdout.write('\n];\n\n');

  stdout.write('const List<int> _mul3 = [');
  for (int x = 0; x < 256; ++x) {
    if (x & 7 == 0) {
      if (x == 8) stdout.write('//');
      stdout.write('\n  ');
